...
import {
  Crown, CheckCircle2, ChevronRight, UserPlus, LogIn, Loader2
} from 'lucide-react';

/**
 * /acceso-founder
 *
 * Stripe redirige aquí tras el pago. Configura en Stripe Dashboard:
 *   Payment Links → tu link → "After payment" → Redirect to URL:
 *   https://matriculapro.ivanimports.es/acceso-founder?success=true
 *
 * Muestra las instrucciones para activar el acceso Founder
 * con el mismo email usado en Stripe.
 *
 * Si ?success=true → pago confirmado (viene de Stripe)
 * Si ?canceled=true → pago cancelado (viene de Stripe cancel_url)
 */
    ...
    return (
      <div className="w-full max-w-[540px]">
        ...
        <motion.div
          ...
          <div className="w-14 h-14 rounded-2xl bg-bg-deep flex items-center justify-center mx-auto mb-4">
            <Crown size={24} className="text-muted" />
          </div>
          ...
          <div className="flex flex-col gap-2">
            <a href="/#precios"
              ...
              <Crown size={14} className="text-accent" /> Volver a ver el acceso Founder
            </a>
            <Link href="/"
              className="text-[12px] text-center text-muted hover:text-ink transition-colors mt-1">
    ...
    <div className="w-full max-w-[580px]">
      ...
      <motion.div
        ...
        <div className="p-7 lg:p-8 text-center relative overflow-hidden"
          ...
          <div className="relative">
            ...
            <h1 className="font-serif italic leading-[1.05] tracking-tight mb-2"
              ...
              {success
                ? '¡Gracias! Tu acceso Founder está en camino.'
                : 'Activa tu acceso Founder Beta'}
            </h1>
            <p className="text-[13px] text-muted-soft leading-relaxed max-w-[420px] mx-auto">
              El acceso se activa automáticamente con el email que usaste en Stripe.
              Crea tu cuenta o inicia sesión con ese mismo email.
            </p>
          </div>
        </div>
        ...
        <div className="px-7 lg:px-8 py-5 border-b border-line bg-bg-deep">
          <div className="text-[10px] tracking-[0.22em] uppercase text-accent-deep mb-3">
            Cómo activar tu acceso
          </div>
          <ol className="space-y-2.5">
            {[
              { n: '1', text: 'Usa el mismo email que introdujiste en el formulario de pago de Stripe.' },
              { n: '2', text: 'Crea tu cuenta gratis o inicia sesión si ya tienes una.' },
              { n: '3', text: 'El sistema detecta tu email y activa el acceso Founder automáticamente.' },
            ].map(item => (
        ...
        <div className="p-7 lg:p-8 flex flex-col gap-3">
          ...
          <Link href="/auth/login"
            ...
            <LogIn size={15} /> Ya tengo cuenta · Iniciar sesión
          </Link>
        </div>

        {/* Nota informativa */}
        <div className="px-7 lg:px-8 pb-6">
          <p className="text-[11.5px] text-muted leading-relaxed text-center">
            Si ya tienes cuenta y no ves el badge Founder tras iniciar sesión,
            espera un minuto y recarga la página. El sistema puede tardar
            unos segundos en sincronizarse.
          </p>
        </div>
      </motion.div>
      ...
      <p className="text-center mt-5 text-[11px] text-muted">
        ...
        {' · '}
        <a href="/app/dashboard" className="hover:underline">Ir al dashboard</a>
      </p>
    </div>
  );
}
...
export default function AccesoFounderPage() {
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <Suspense fallback={
        <div className="rounded-2xl p-8 bg-surface border border-line flex items-center gap-3">
          <Loader2 size={20} className="animate-spin text-accent" />
          <span className="text-[13px] text-ink-soft">Cargando…</span>
        </div>
      }>
