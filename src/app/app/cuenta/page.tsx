...
export default function AccountPage() {
  ...
  const downloadData = () => action('download', async () => { if (!user) throw new Error('No hay sesión.'); const supabase = createSupabaseBrowserClient(); const [checks, cases, deletion] = await Promise.all([supabase.from('free_vehicle_checks').select('id, risk_level, rule_version, created_at').eq('user_id', user.id).is('deleted_at', null), supabase.from('registration_cases').select('id, title, status, created_at, updated_at').eq('user_id', user.id).is('deleted_at', null), supabase.from('account_deletion_requests').select('id, status, requested_at, resolved_at').eq('user_id', user.id)]); const exportData = { exportedAt: new Date().toISOString(), account: { id: user.id, email: user.email, displayName: profile?.display_name ?? null }, access: { tier: access.tier, mode: access.mode, license: access.license }, freeChecks: checks.data ?? [], cases: cases.data ?? [], deletionRequests: deletion.data ?? [] }; const url = URL.createObjectURL(new Blob([JSON.stringify(exportData, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `matriculapro-datos-${new Date().toISOString().slice(0, 10)}.json`; anchor.click(); URL.revokeObjectURL(url); return 'Exportación preparada.'; });
  const logout = async (scope: 'local' | 'global') => { setBusy(scope); await signOut(scope); window.location.replace('/'); };

  return <div className="mx-auto max-w-[1020px] px-5 pb-16 pt-7 lg:px-8"><header><div className="text-[9.5px] uppercase tracking-[.2em] text-accent-deep">Cuenta</div><h1 className="mt-1 font-serif text-[38px] text-ink">Perfil, licencia y privacidad</h1><p className="mt-2 max-w-2xl text-[12.5px] leading-relaxed text-ink-soft">Gestiona los datos básicos y registra solicitudes sensibles sin borrar inmediatamente información que pueda estar sujeta a conservación legal.</p></header>
