    ...
    <header className="sticky top-0 z-50 transition-all"
      ...
      <div className="max-w-[1280px] mx-auto px-5 lg:px-8 h-[72px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={demoHref} className="hidden md:inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-[12px]" style={{ background: tokens.color.bgDeep, color: tokens.color.inkSoft }}>
          ...
          <Link href={buyHref}
            ...
            Acceder al curso <ChevronRight size={13} />
          </Link>
        </div>
      </div>
    </header>
        ...
        <div className="grid lg:grid-cols-[1.1fr_1fr] gap-10 lg:gap-16 items-center">
          ...
          <div>
            ...
            <motion.p
              ...
              MatriculaPRO es una <strong style={{ color: tokens.color.ink }}>guía interactiva con simuladores, checklists y recorridos visuales</strong> para entender el proceso de matriculación en España paso a paso. No es un PDF — es una experiencia práctica.
            </motion.p>
...
const PricingSection: React.FC<PricingSectionProps> = ({ demoHref, buyHref, premiumHref }) => {
  const hrefFor = (id: PricingPlan['id']): string => {
    if (id === 'demo') return demoHref;
    if (id === 'completo') return buyHref;
    return premiumHref;
  };
  return (
    <section id="precios" className="py-20 lg:py-28" style={{ background: tokens.color.bg }}>
      <div className="max-w-[1280px] mx-auto px-5 lg:px-8">
        <div className="text-center mb-12">
          <div className="text-[10.5px] tracking-[0.22em] uppercase mb-3" style={{ color: tokens.color.accentDeep }}>Precios</div>
          <h2 className="max-w-[680px] mx-auto" style={{ fontFamily: 'Instrument Serif, serif', fontSize: 'clamp(28px, 3.6vw, 48px)', color: tokens.color.ink, letterSpacing: '-0.01em', lineHeight: 1.1 }}>
            Empieza gratis. Si te convence, <span style={{ fontStyle: 'italic', color: tokens.color.accent }}>pagas una vez</span>.
          </h2>
          <p className="mt-4 max-w-[560px] mx-auto text-[14.5px] leading-relaxed" style={{ color: tokens.color.inkSoft }}>
            Sin suscripciones. Sin renovaciones. Acceso permanente y actualizaciones incluidas.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-5 items-stretch">
          {PRICING.map(plan => (
            <PricingCard key={plan.id} plan={plan} href={hrefFor(plan.id)} />
          ))}
        </div>

        {/* Cupón 20% */}
        <div className="mt-8 rounded-2xl p-5 flex items-start gap-4 max-w-[820px] mx-auto"
             style={{ background: tokens.color.surface, border: `1px dashed ${tokens.color.accent}` }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: tokens.color.accentSoft, color: tokens.color.accentDeep }}>
            <Sparkles size={18} />
          </div>
          <div>
            <div className="text-[10.5px] tracking-[0.22em] uppercase mb-1" style={{ color: tokens.color.accentDeep }}>Bonus al comprar</div>
            <div className="text-[14px]" style={{ color: tokens.color.ink }}>
              <strong>Cupón del 20%</strong> para futuros cursos de Ivan Imports (importación, subastas, búsqueda de coches…). Un solo uso, no acumulable.
            </div>
          </div>
        </div>
      </div>
...
const PricingCard: React.FC<{ plan: PricingPlan; href: string }> = ({ plan, href }) => {
  ...
  return (
    <motion.div
      ...
      className="rounded-[22px] p-6 lg:p-7 flex flex-col relative overflow-hidden"
      style={{
        background: isPremium ? 'linear-gradient(135deg, #0B1F3A 0%, #16335E 100%)' : tokens.color.surface,
        color: isPremium ? '#fff' : tokens.color.ink,
        border: `1px solid ${isFeatured ? tokens.color.accent : isPremium ? 'rgba(200,134,46,0.3)' : tokens.color.line}`,
        boxShadow: isFeatured ? `0 0 0 4px ${tokens.color.accent}22, ${tokens.shadow.lg}` : tokens.shadow.sm,
        transform: isFeatured ? 'translateY(-8px)' : 'none',
      }}>
      {isPremium && <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full opacity-30 blur-3xl" style={{ background: 'radial-gradient(circle, #C8862E 0%, transparent 70%)' }} />}

      {plan.badge && (
        <div className="relative inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9.5px] tracking-[0.18em] uppercase mb-3 self-start"
             style={{ background: isPremium ? tokens.color.accent : tokens.color.accentSoft, color: isPremium ? tokens.color.ink : tokens.color.accentDeep, fontWeight: 700 }}>
          {plan.badge}
        </div>
      )}

      <div className="relative">
        <h3 style={{ fontFamily: 'Instrument Serif, serif', fontSize: 26, letterSpacing: '-0.01em', lineHeight: 1.1 }}>
          {plan.name}
        </h3>
        <p className="mt-2 text-[12.5px] leading-relaxed" style={{ color: isPremium ? '#B4BECE' : tokens.color.inkSoft }}>
          {plan.desc}
        </p>
        ...
        <div className="mt-5 mb-1 flex items-baseline gap-2">
          {plan.priceOriginal && (
            <span className="text-[14px] line-through" style={{ color: isPremium ? '#7A869A' : tokens.color.muted }}>{plan.priceOriginal}€</span>
          )}
          <span style={{ fontFamily: 'Instrument Serif, serif', fontSize: 56, lineHeight: 1, color: isPremium ? '#fff' : tokens.color.ink }}>
            {plan.price}
          </span>
          <span className="text-[18px]" style={{ color: tokens.color.accent }}>€</span>
        </div>
        <div className="text-[11px]" style={{ color: isPremium ? '#7A869A' : tokens.color.muted }}>{plan.period}</div>

        {/* CTA */}
        <Link href={href}
          className="mt-5 w-full inline-flex items-center justify-center gap-1.5 px-4 py-3 rounded-full text-[13px] font-medium transition-transform hover:scale-[1.02]"
          style={{
            background: plan.ctaSecondary ? 'transparent' : isPremium ? tokens.color.accent : tokens.color.ink,
            color: plan.ctaSecondary ? tokens.color.ink : isPremium ? tokens.color.ink : '#fff',
            border: plan.ctaSecondary ? `1px solid ${tokens.color.ink}` : 'none',
          }}>
        ...
        <div className="mt-6 space-y-2">
          {plan.includes.map((item, i) => (
            <div key={i} className="flex items-start gap-2 text-[12.5px]" style={{ color: isPremium ? '#E4E9F2' : tokens.color.inkSoft }}>
              <Check size={12} className="shrink-0 mt-0.5" style={{ color: tokens.color.accent }} />
              <span>{item}</span>
            </div>
          ))}
          {plan.excludes && plan.excludes.map((item, i) => (
            <div key={`x${i}`} className="flex items-start gap-2 text-[12.5px] opacity-60">
              <X size={12} className="shrink-0 mt-0.5" style={{ color: tokens.color.muted }} />
              <span style={{ color: tokens.color.muted }}>{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-[28px] p-10 lg:p-16 text-center relative overflow-hidden"
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full mb-5"
               style={{ background: 'rgba(200,134,46,0.18)', color: tokens.color.accent }}>
            <Zap size={11} />
            <span className="text-[10.5px] tracking-[0.18em] uppercase font-semibold">Preventa · 89 €</span>
          </div>
          ...
          <div className="mt-8 flex items-center justify-center gap-3 flex-wrap">
            <Link href={buyHref}
              ...
              Comprar curso completo · 89 € <ChevronRight size={14} />
            </Link>
            <Link href={demoHref}
...
export default function Landing({
  ...
  return (
    <div style={{ background: tokens.color.bg, color: tokens.color.ink, fontFamily: '"Geist", system-ui, -apple-system, sans-serif' }}>
      ...
      <PricingSection demoHref={demoHref} buyHref={buyHref} premiumHref={premiumHref} />
