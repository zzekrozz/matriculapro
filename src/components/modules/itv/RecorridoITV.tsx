...
import {
  ...
  CheckCircle2, AlertTriangle, GraduationCap, type LucideIcon
} from 'lucide-react';
      ...
      <div className="px-5 lg:px-8 pt-6 pb-12 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6 items-start">
              ...
              <div className="px-5 py-4 border-t flex items-center gap-1.5 overflow-x-auto" style={{ borderColor: tokens.color.line, background: tokens.color.surfaceAlt }}>
                ...
                {isDemo && (
                  <div className="ml-2 inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] shrink-0" style={{ background: tokens.color.bgDeep, color: tokens.color.muted }}>
                    <Lock size={10} /> +6 pasos en la versión completa
                  </div>
                )}
              </div>
            </div>
          ...
          <motion.div key={step.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}
            ...
            {isDemo && isLast && (
              <div className="mx-6 mb-6 rounded-xl p-4" style={{ background: 'linear-gradient(135deg, #0B1F3A 0%, #16335E 100%)', color: '#fff' }}>
                <div className="text-[10px] tracking-[0.22em] uppercase mb-1" style={{ color: tokens.color.accent }}>Versión completa</div>
                <p className="text-[12.5px] leading-relaxed" style={{ color: '#B4BECE' }}>
                  Has visto los primeros 5 pasos. En el curso completo verás también luces traseras, dirección, ruedas, frenos en rodillos, emisiones y resultado final.
                </p>
              </div>
            )}
          </motion.div>
