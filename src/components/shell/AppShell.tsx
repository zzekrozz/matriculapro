...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const hiddenAuth = access.publicBeta;
  ...
  return <div className="min-h-screen overflow-x-hidden bg-bg">
    ...
    <div className="flex"><aside className={cn('print-shell-hidden fixed inset-y-0 left-0 z-30 flex w-[86vw] max-w-[310px] flex-col bg-ink text-white transition-transform lg:static lg:w-[274px] lg:shrink-0 lg:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
      ...
      <div className="border-t border-white/5 px-3 pb-4 pt-3">{user?.email && !hiddenAuth && <div className="truncate px-3 pb-2 text-[9.5px] text-muted">{user.email}</div>}{!hiddenAuth && <LogoutButton />}<Link href="/legal/aviso-fiscal-tecnico" className="block px-3 pt-2 text-[9.5px] text-muted hover:text-white">Alcance fiscal y técnico</Link></div>
    </aside>
...
function PublicBetaBanner() { return <div className="print-shell-hidden border-b border-accent/25 bg-accent-soft px-5 py-2.5 text-[11px] text-accent-deep lg:px-8"><strong>MatriculaPro Beta.</strong> Acceso abierto sin cuenta; los datos que guardes permanecen únicamente en este navegador.</div>; }
