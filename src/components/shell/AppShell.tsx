...
interface NavItem {
  ...
  /** Si true, requiere founder o full para acceder con funcionalidad completa */
...
const NAV_ITEMS: NavItem[] = [
  ...
  { href: '/app/recorrido-itv',       icon: Car,             label: 'Recorrido ITV', code: 'M.06', badge: 'hot', requiresFounder: true },
      ...
      <div className="flex">
        ...
        >
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                  ...
                  {item.badge === 'hot' && !isLocked && <Sparkles size={10} className="text-accent shrink-0" />}
                  {item.badge === 'premium' && <span className="text-[8.5px] font-semibold text-accent shrink-0">PRO</span>}
                  {isLocked && <Lock size={10} className="text-muted shrink-0" />}
                </Link>
              );
            })}
          ...
          <div className="px-3 pb-4 border-t border-white/5 pt-4 space-y-1">
            <Link href="/auth/login" className="text-[10.5px] text-muted hover:text-white px-3 flex items-center gap-1.5 py-1">
              → Iniciar sesión
            </Link>
    ...
    <div className="sticky top-0 z-20 px-5 lg:px-8 py-3 flex items-center gap-3 justify-between flex-wrap"
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        <span className="text-[12px] leading-tight text-accent-deep truncate">
          Puedes ver la ruta general gratis. Desbloquea <strong>Founder Beta · 49&nbsp;€</strong> para practicar y conservar acceso de por vida.
        </span>
      </div>
