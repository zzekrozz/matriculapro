...
export function LoginForm({ nextUrl = '/app/dashboard' }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (err) {
      // Error visible — siempre resetear loading para no quedarse bloqueado
      setError(err);
      setLoading(false);
      return;
    }

    // Login OK. onAuthStateChange ya actualizó el perfil y la cookie.
    // Redirigir con replace para que el servidor vea la sesión actualizada.
    console.log('[LoginForm] OK → redirigiendo a:', nextUrl);
    window.location.replace(nextUrl);

    // Fallback: si la navegación tarda más de 8 segundos, desbloquear botón
