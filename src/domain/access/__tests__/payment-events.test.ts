...
function checkout(overrides: Partial<VerifiedCheckoutSnapshot> = {}): VerifiedCheckoutSnapshot {
  return {
    ...
    invoiceCurrency: 'eur',
    invoiceCountry: 'ES',
    invoicePriceId: 'price_particular_6m',
    invoiceAutomaticTaxEnabled: true,
...
describe('trusted Checkout validation', () => {
  ...
  it('rejects every client-manipulable monetary field', () => {
    const cases: Array<[Partial<VerifiedCheckoutSnapshot>, string]> = [
      ...
      [{ automaticTaxEnabled: false }, 'automatic_tax_incomplete'],
      [{ automaticTaxStatus: 'requires_location_inputs' }, 'automatic_tax_incomplete'],
      [{ taxBehavior: 'exclusive' }, 'tax_behavior_mismatch'],
      [{ taxAmountCents: 0, subtotalExcludingTaxCents: 10_000 }, 'tax_breakdown_mismatch'],
      [{ taxAmountCents: 1_735 }, 'tax_breakdown_mismatch'],
      [{ invoiceId: null }, 'invoice_missing'],
      [{ invoiceStatus: 'open' }, 'invoice_mismatch'],
      [{ invoicePriceId: 'price_wrong' }, 'invoice_mismatch'],
      [{ invoiceAutomaticTaxStatus: 'failed' }, 'invoice_mismatch'],
      [{ invoiceTaxBehavior: 'exclusive' }, 'invoice_mismatch'],
      [{ invoiceSubtotalExcludingTaxCents: 8_263 }, 'invoice_mismatch'],
      [{ invoiceTaxAmountCents: 1_735 }, 'invoice_mismatch'],
      [{ invoiceTotalIncludingTaxCents: 9_999 }, 'invoice_mismatch'],
    ];
