...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Email del administrador/creador — usado para diagnóstico en consola (no se expone en UI)
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    if (error) {
      console.warn('[MatriculaPRO] fetchProfile error:', error.message);
    }

    const profile = data as UserProfile ?? null;

    // Log de diagnóstico — visible en consola del navegador durante desarrollo
    if (profile) {
      const founderTag = profile.founder_number
        ? `Founder #${String(profile.founder_number).padStart(4, '0')}`
        : 'sin número';
      console.log(
        `[MatriculaPRO] 👤 ${profile.email} | nivel: ${profile.access_level} | ${founderTag}`
      );
      if (profile.email === ADMIN_EMAIL) {
        console.log('[MatriculaPRO] 🔑 Admin detectado — nivel debería ser full/founder con #0001');
      }
    }
