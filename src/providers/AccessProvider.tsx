...
// El primer número disponible para compradores. El #0001 está reservado para el admin.
...
export function AccessProvider({ children }: { children: ReactNode }) {
  ...
  // Sincronizar con perfil Supabase cuando llegue: Supabase es la fuente de verdad.
  // También sincroniza la cookie para que el middleware vea el nivel correcto.
  useEffect(() => {
    if (profile) {
      setLocalLevel(profile.access_level);
      if (profile.founder_number) setFounderNumber(profile.founder_number);
      // Sincronizar cookie con el nivel real de Supabase
  ...
  // Nivel efectivo con lógica clara:
  // 1. Si Supabase tiene perfil → usar ese nivel (fuente de verdad)
  // 2. Si hay usuario pero auth aún carga → usar localStorage (evita parpadeo a Explorer)
  // 3. Si hay usuario pero sin perfil todavía → usar localStorage (puede ser founder si ya se activó)
  // 4. Si no hay usuario → usar localStorage (mock/dev) o visitor
  const effectiveLevel: AccessLevel = (() => {
    if (profile) return profile.access_level;           // Supabase manda
    if (authLoading) return localLevel;                 // Cargando: no asumir explorer
    if (user) return localLevel === 'visitor'           // Sesión sin perfil:
      ? 'explorer'                                      //   si era visitor → explorer mínimo
      : localLevel;                                     //   si tenía founder en local → respetarlo
    return localLevel;                                  // Sin sesión: mock/dev o visitor
  })();
